...
# first get summary stats for
# flipper length
# and update grouping vars
# for species and year
penguins_fm <- 
  summarySE(penguins, 
            measurevar="flipper_length_mm", 
            groupvars=c("species","year"))

# clean out any NAs, missing data
penguins_fmc <- penguins_fm %>%
  filter(complete.cases(.))

# now make plot with updated summary stats
ggplot(penguins_fmc, 
       aes(x=year, y=flipper_length_mm, fill=species)) + 
  geom_bar(position=position_dodge(), stat="identity") +
...
# by using the percent() function from the scales package
ggplot(penguins_tab1.df, 
       aes(x = sex, y = count, fill = island)) +
  ...
  labs(
    x = "Biological Sex",
    y = "Relative Percent",
    title = "Percentage of Island within Biological Sex",
...
# change names
names(penguins_tab1.df) <-
  c("species", "island", "count")

# make a little better
ggplot(penguins_tab1.df, 
       aes(x = species, y = count, fill = island)) +
  geom_col(colour = "black", position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(palette = "Pastel1") +
  labs(
    x = "Species",
    y = "Relative Percent",
    title = "Percentage of Island within Species",
    fill = "Island Surveyed"
  )

# flip relative percents
# look at % of species within each island
ggplot(penguins_tab1.df, 
  ...
  labs(
    x = "Island",
    y = "Relative Percent",
    title = "Percentage of Species on each Island",
    fill = "Species"
  )
